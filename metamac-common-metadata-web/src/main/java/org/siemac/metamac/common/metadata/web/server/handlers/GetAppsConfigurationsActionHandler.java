    ...
    public GetAppsConfigurationsResult executeSecurityAction(GetAppsConfigurationsAction action) throws ActionException {
        ...
        try {
            switch (action.getType()) {
                case SYSTEM:
                    configurations = commonMetadataServiceFacade.findDataConfigurationsOfSystemProperties(ServiceContextHolder.getCurrentServiceContext());
                    break;
                case DEFAULT_VALUES:
                    configurations = commonMetadataServiceFacade.findDataConfigurationsOfDefaultValues(ServiceContextHolder.getCurrentServiceContext());
                    break;
                default:
